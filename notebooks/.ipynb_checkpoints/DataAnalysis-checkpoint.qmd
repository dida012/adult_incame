...
We load the cleaned version of the UCI Adult Income Dataset from the processed data directory into a Pandas DataFrame. The \`head(10)\` function shows the first ten records, giving a glimpse into the data columns such as \`age\`, \`workclass\`, \`education\_num\`, etc.
...
\- The dataset includes both \*\*numerical\*\* (e.g., \`age\`, \`hours\_per\_week\`) and \*\*categorical\*\* variables (e.g., \`sex\`, \`education\_level\`).
